...
void RNG::init( unsigned int* seeds )
{
    ...
    for( int i = 0; i < RNG_STATE_SIZE; ++i ) state[i] = new unsigned int[num_threads];

    for( int t = 0; t < num_threads; ++t ) {
        have_spare_gaussian[t] = false;
        state[0][t] = seeds[t];
        for( int i = 1; i < RNG_STATE_SIZE; ++i ) {
            state[i][t] = ( int )( 1812433253
                                   * ( state[i - 1][t] ^ ( state[i - 1][t] >> 30 ) ) + i );
        }
...
void RNG::make_random()
{
    make_random( omp_get_thread_num() );
}
